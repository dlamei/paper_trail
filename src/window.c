...
VkExtent2D glfw_get_nonzero_window_size(GLFWwindow *window) {
	i32 width, height;
...
void glfw_wait_if_minimized(GLFWwindow *window) {
	i32 width, height;
	glfwGetFramebufferSize(window, &width, &height);
...
void framebuffer_resized_callback(GLFWwindow *window, i32 width, i32 height) {
	PapertrailWindowCallbackFn *ptrs = glfwGetWindowUserPointer(window);
	VkContext *c = ptrs->p_context;
	PapertrailRenderpass *rp = ptrs->p_renderpass;

	rp->swapchain_create_info.image_extent = glfw_get_nonzero_window_size(window);
...
VkContext vk_context_init() {
	...
	GLFWwindow *p_window = glfwCreateWindow(1000, 800, "Papertrail", NULL, NULL);
    glfwSetWindowSizeLimits(p_window, 1, 1, GLFW_DONT_CARE, GLFW_DONT_CARE);
	...
	u32 req_extension_index = 0;
	{
		...
		for (u32 i = 0; i < MIN(glfw_extension_count, MAX_PROPERTIES_LEN); i++) {
			required_extensions[req_extension_index++] = glfw_extensions[i];
		}
	...
	VkSurfaceKHR surface;
	VK_CHECK(glfwCreateWindowSurface(instance, p_window, NULL, &surface));
...
PapertrailRenderpass ptrail_renderpass_init(const VkContext *c) {
	VkPipelineLayout pipeline_layout;
	VkRenderPass renderpass;
	VkPipeline pipeline;
	Swapchain swapchain;
	...
	SwapchainCreateInfo swapchain_create_info = {
			.image_extent = glfw_get_nonzero_window_size(c->p_window),
...
void ptrail_render_frame(const VkContext *c, PapertrailRenderpass *rp) {
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR) {
		rp->swapchain_create_info.image_extent = glfw_get_nonzero_window_size(c->p_window);
...
void window_refresh_callback(GLFWwindow *window) {
	...
	VkContext *c = ptrs->p_context;
	PapertrailRenderpass *rp = ptrs->p_renderpass;
	ptrail_render_frame(c, rp);
}

void run() {
	VkContext c = vk_context_init();
	PapertrailRenderpass rp = ptrail_renderpass_init(&c);
	...
	glfwSetFramebufferSizeCallback(c.p_window, framebuffer_resized_callback);
	glfwSetWindowRefreshCallback(c.p_window, window_refresh_callback);
	glfwSetWindowUserPointer(c.p_window, &callback_ptrs);

	/// RENDER LOOP ///
	while (!glfwWindowShouldClose(c.p_window)) {
		glfw_wait_if_minimized(c.p_window);
		ptrail_render_frame(&c, &rp);
		glfwPollEvents();
	}
